from models.UNet7 import Trans, Regs, Regs2, Reg2,  SpatialTransformer
from models.transform import ModalEncoder, MorphEncoder, Decoder, Decoder2
...
class PMARNet3(torch.nn.Module):
    def __init__(self, args):
        super(PMARNet3, self).__init__()
        self.args = args
        ...
        #model加载
        self.modal_encoder = ModalEncoder(inshape=args.img_shape, types=args.types, freeze=False).to(self.device)
        self.morph_encoder = MorphEncoder(inshape=args.img_shape, freeze=False).to(self.device)
        self.gen = Decoder2().to(self.device)
        self.reg = Regs(VxmDense, args.n_cas, args.img_shape).to(self.device)
        ...
        if True:
            ...
            self.optimizerE1 = torch.optim.Adam(filter(lambda p: p.requires_grad, self.modal_encoder.parameters()), lr=3e-4, betas=(args.beta1, 0.999))
            self.optimizerE2 = torch.optim.Adam(filter(lambda p: p.requires_grad, self.morph_encoder.parameters()), lr=3e-4, betas=(args.beta1, 0.999))
            ...
            self.optimizerE1 = torch.optim.RMSprop( filter(lambda p: p.requires_grad, self.modal_encoder.parameters()), lr = args.lrG)
            self.optimizerE2 = torch.optim.RMSprop( filter(lambda p: p.requires_grad, self.morph_encoder.parameters()), lr = args.lrG)
    ...
    def load_model(self, load_path, is_best=True):
        ...
        self.modal_encoder.load_state_dict(ckp['modal_encoder'])
        self.morph_encoder.load_state_dict(ckp['morph_encoder'])
        ...
        self.optimizerE1.load_state_dict(ckp['optimizerE1'])
        self.optimizerE2.load_state_dict(ckp['optimizerE2'])
    ...
    def save_model(self, epoch=0, is_best=False):
        if is_best:
            ckp = {}
            ckp['modal_encoder'] = self.modal_encoder.state_dict()
            ckp['morph_encoder'] = self.morph_encoder.state_dict()
        ...
        ckp['optimizerE1'] = self.optimizerE1.state_dict()
        ckp['optimizerE2'] = self.optimizerE2.state_dict()
    ...
    def write_tb(self, epoch):
        for k, v in self.recoder.items():
            loss_np = np.array(v)
            self.writer.add_scalar(f'{k}', loss_np.mean(), epoch)
    ...
    def forward(self, movings, fixeds, m_type, f_type):
        self.modal_encoder.set_input(movings, fixeds, m_type, f_type)
        self.morph_encoder.set_input(movings, fixeds, m_type, f_type)
        
        s_m_proto, s_f_proto = self.modal_encoder.forward1()
        s_m_content, s_f_content = self.morph_encoder.forward1()
        self.gen_mf = self.gen(s_m_proto, s_f_proto, s_m_content, s_f_content)
        self.flows, self.warpeds, self.warpeds_m, self.residuals = self.reg(movings, fixeds, self.gen_mf)
        return self.flows, self.warpeds, self.gen_mf
    
    def infer(self, movings, fixeds):        
        flows, warpeds = self.reg.infer(movings, fixeds)
        return flows, warpeds
    
    def backward_dis(self, fixeds, movings):
        dis_loss = self.args.w_dis * (self.net_D.dis_loss(self.gen_mf.detach(), fixeds)
                                        + self.net_D.dis_loss(self.gen_mf.detach(), movings))*0.5
        dis_loss.backward()
        self.recoder['train/dis_loss'].append(self.args.w_dis * dis_loss.item())

    def backward_style_encoder(self):
        total_loss = self.args.w_proto*self.modal_encoder.proto_loss + self.args.w_contra*self.modal_encoder.contra_loss
        total_loss.backward(retain_graph=True)
        self.recoder['train/proto_loss'].append(self.args.w_proto*self.modal_encoder.proto_loss.item())
        self.recoder['train/contra_loss'].append(self.args.w_contra*self.modal_encoder.contra_loss.item())

    def backward_gen(self):
        # style and content loss
        style_loss = self.modal_encoder.forward2(self.gen_mf)
        content_loss = self.morph_encoder.forward2(self.gen_mf)
        # content_loss = self.ncc_loss(self.morph_encoder.moving, self.gen_mf)+1
        ...
        total_loss.backward(retain_graph=True)
        self.recoder['train/style_loss'].append( self.args.w_style * style_loss.item())
        self.recoder['train/content_loss'].append(self.args.w_content * content_loss.item())
    ...
    def backward_reg(self, fixeds):
        ...
        for i in range(len(self.warpeds)):
            ...
            sim_loss = sim_loss + ((self.warpeds[i] - fixeds).pow(2) * (self.residuals[i]+1)).mean()
            # cur_sim = (self.warpeds[i] - fixeds).pow(2)
            # sim_loss = sim_loss - (self.residuals[i]- cur_sim) * cur_sim
        # sim_loss = sim_loss / len(self.warpeds)
        ...
        total_loss.backward()
        self.recoder['train/sim_loss'].append(self.args.w_sim * sim_loss.item())
        self.recoder['train/smooth_loss'].append(self.args.w_smooth * smooth_loss.item())
        self.recoder['train/gan_loss'].append(self.args.w_gan * gan_loss.item())
        # self.recoder['train/proto_loss'].append(self.encoder.proto_loss.item())
        # self.recoder['train/contra_loss'].append(self.encoder.contra_loss.item())
        self.recoder['train/total_loss'].append(total_loss.item())

    def optimize(self, epoch, movings, fixeds, m_type, f_type):
        self.global_step += 1
        self.forward(movings, fixeds, m_type, f_type)
        
        self.set_requires_grad([self.modal_encoder, self.morph_encoder, self.gen, self.reg], False)
        # backward dis
        if self.global_step % args.dis_gap == 0: # 判别器隔gap轮更新一次
        # if epoch % args.dis_gap == 0: # 判别器隔gap轮更新一次
            self.set_requires_grad([self.net_D], True)
            self.optimizerD.zero_grad()
            self.backward_dis(fixeds, movings)
            torch.nn.utils.clip_grad_norm_(list(self.net_D.parameters()), max_norm=2, norm_type=2)
            self.optimizerD.step()
        
        # backward encoder
        self.set_requires_grad([self.net_D], False) # 固定dis参数
        self.set_requires_grad([self.modal_encoder], True) 
        self.optimizerE1.zero_grad()
        self.backward_style_encoder() # 更新模态编码器

        # backward decoder
        self.set_requires_grad([self.modal_encoder], False) # 固定dis参数
        self.set_requires_grad([self.gen], True)
        self.optimizerG.zero_grad()
        self.backward_gen() # 只更新解码器

        torch.nn.utils.clip_grad_norm_(list(self.gen.parameters()), max_norm=10, norm_type=2)

        # backward gen and reg
        self.set_requires_grad([self.morph_encoder, self.reg], True) # 优化reg, gen
        self.optimizerE2.zero_grad()
        self.optimizerR.zero_grad()
        self.backward_reg(fixeds) # 跟新配准模块+解码器+形态编码器
        
        torch.nn.utils.clip_grad_norm_(list(self.reg.parameters()), max_norm=10, norm_type=2)
        
        self.optimizerE1.step()
        self.optimizerE2.step()
    ...
    #     # backward gen and reg
    #     self.set_requires_grad([self.morph_encoder, self.modal_encoder, self.reg], True) # 优化reg, gen
    #     self.optimizerE1.zero_grad()
    #     self.optimizerE2.zero_grad()
    #     self.optimizerR.zero_grad()
    #     self.backward_reg(fixeds) # 跟新配准模块+解码器+形态编码器
    ...
    def train_modal_encoder(self):
        abdomen_dataset = AbdomenDataset2(Path(args.train_dir),same_skip=True)
        self.train_loader = Data.DataLoader(abdomen_dataset,batch_size=args.batch_size, shuffle=False, num_workers=0, drop_last=True)
    ...
    def train_model(self):
        abdomen_train = AbdomenDataset1(Path(args.train_dir), fixed_seqs=['UP'], moving_seqs=['CMP','NP'], same_skip=True)
        abdomen_val = AbdomenDataset2(Path(args.test_image_dir), Path(args.test_label_dir), fixed_seqs=['UP'], moving_seqs=['CMP','NP'], same_skip=True)

        # brats_train = BraTSRegDataset2(Path('E:\\datasets\\BRAST2018\\MICCAI_BraTS_2018_Data_Training\\LGG'), 
        #                             fixed_seqs=['t1','t21','t1ce1'], moving_seqs=['t1','t21','t1ce1'],  resize=(64, 64, 64), label='WT', flag=0)
        # brats_val = BraTSRegDataset2(Path('E:\\datasets\\BRAST2018\\MICCAI_BraTS_2018_Data_LGG_Test'), 
        #                             fixed_seqs=['t1','t21','t1ce1'], moving_seqs=['t1','t21','t1ce1'], resize=(64, 64, 64), label='WT', flag=1)
        
        # chaos_train = CHAOS4(Path('E:\\datasets\\CHAOS\\Train_Sets1'), resize=(32,128,128), flag=0)
        # chaos_val = CHAOS4(Path('E:\\datasets\\CHAOS\\Test_Sets1'), resize=(32,128,128), flag=1)

        self.train_loader = Data.DataLoader(abdomen_train, batch_size=args.batch_size, shuffle=True, num_workers=0, drop_last=True)             
        self.test_dataloader = Data.DataLoader(abdomen_val, batch_size=args.test_batch_size, shuffle=False, num_workers=0, drop_last=True)
    ...
    def test_model(self, epoch):
        ...
        with torch.no_grad():
            ...
            for movings, fixeds, moving_lab, fixed_lab, m_type, f_type in self.test_dataloader:
                ...
                # flows, warpeds, gen_mf = self.infer(moving_img, fixed_img)
                flows, warpeds, gen_mf = self.forward(moving_img, fixed_img, m_type, f_type)

                warped_lab = moving_lab
                for i in range(len(flows)):
                    warped_lab = self.stn(warped_lab, flows[i])

                if False and jpg_path is not None:
                ...
                w_f = (warped_lab>0)*0.5 + (fixed_lab)*0.5
                self.writer.add_image('test_label/overlap_warped', w_f[0,0,ran_id:ran_id+1,:,:], epoch*gap+step)
                m_f = (moving_lab>0)*0.5 + (fixed_lab)*0.5
                self.writer.add_image('test_label/overlap_real', m_f[0,0,ran_id:ran_id+1,:,:], epoch*gap+step)
                ...
                self.writer.add_image('test_image/trans_img', gen_mf[0,0,ran_id:ran_id+1,:,:], epoch*gap+step)
                for i in range(len(warpeds)):
                    self.writer.add_image(f'test_image/wapred_{i}', warpeds[i][0,0,ran_id:ran_id+1,:,:], epoch*gap+step)
                    flow_vis = self.flowshow.show(self.stn, flows[i])
        ...
        self.modal_encoder.train()
        self.morph_encoder.train()
        self.gen.train()
        self.reg.train()

        mean_dice = key_max / len(dice_dict.keys())
        self.recoder['val/Dice'] = mean_dice
        return mean_dice
...
class PMARNet31(PMARNet3):
    def __init__(self, args):
        super(PMARNet31, self).__init__(args)
    ...
    #     total_loss = self.args.w_proto*self.modal_encoder.proto_loss + self.args.w_contra*self.modal_encoder.contra_loss + self.args.w_morp * morp_loss
        
    #     total_loss.backward(retain_graph=True)
    #     self.recoder['train/proto_loss'].append(self.args.w_proto * self.modal_encoder.proto_loss.item())
    #     self.recoder['train/contra_loss'].append(self.args.w_contra * self.modal_encoder.contra_loss.item())
    #     self.recoder['train/morp_loss'].append(self.args.w_morp * morp_loss.item())

    # def backward_style_encoder(self):
    #     total_loss = self.args.w_infonce * self.modal_encoder.nce_loss
    #     total_loss.backward(retain_graph=True)
    #     self.recoder['train/infonce_loss'].append(self.args.w_infonce * self.modal_encoder.nce_loss.item())
    ...
    def backward_reg(self, fixeds):
        ...
        smooth_loss = 0
        for i in range(len(self.warpeds)):
            tmp_sim_loss = torch.nn.functional.mse_loss(self.warpeds[i], fixeds)
            tmp_smooth_loss = losses.reg_loss(fixeds, [self.flows[i]])
            
            sim_loss = sim_loss + tmp_sim_loss
            smooth_loss = smooth_loss + tmp_smooth_loss

            self.recoder[f'train/sim_loss{i}'].append(self.args.w_sim * tmp_sim_loss.item())
            self.recoder[f'train/smooth_loss{i}'].append(self.args.w_smooth * tmp_smooth_loss.item())

        total_loss = args.w_gan*gan_loss + self.args.w_sim*sim_loss + self.args.w_smooth*smooth_loss 
    ...
    def optimize(self, epoch, movings, fixeds, m_type, f_type):
        ...
        # backward encoder
        self.set_requires_grad([self.net_D], False) # 固定dis参数
        self.set_requires_grad([self.modal_encoder], True) # 更新解码器和编码器
        self.optimizerE1.zero_grad()
        self.backward_style_encoder() # 更新编码器（原型+对比损失）

        # backward gen and reg
        self.set_requires_grad([self.modal_encoder], False) # 固定模态编码器参数
        self.set_requires_grad([self.morph_encoder, self.gen, self.reg], True) # 优化reg, gen, 形态编码器
        self.optimizerE2.zero_grad()
        self.optimizerG.zero_grad()
        self.optimizerR.zero_grad()
        self.backward_reg(fixeds) # 更新解码器+形态编码器+配准模块

        # backward decoder 校正解码器
        self.set_requires_grad([self.morph_encoder, self.reg], False)
        self.backward_gen(fixeds) # 更新解码器
        ...
        self.set_requires_grad([self.modal_encoder, self.morph_encoder, self.reg], True)
        ...
        if self.global_step % self.args.tb_save_freq == 0:
            ...
            for i in range(len(self.residuals)):
                self.writer.add_image(f'train/residuals_{i}', (self.residuals[i][0,0,ran_id:ran_id+1,:,:]), self.tb_save_step)
